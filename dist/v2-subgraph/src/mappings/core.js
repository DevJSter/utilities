...
import { Bundle, Burn as BurnEvent, Mint as MintEvent, Pair, Swap as SwapEvent, Token, Transaction, UniswapFactory, } from '../types/schema';
...
function isCompleteMint(mintId) {
...
export function handleTransfer(event) {
    ...
    if (from.toHexString() == ADDRESS_ZERO) {
        ...
        if (mints.length === 0 || isCompleteMint(mints[mints.length - 1])) {
            let mint = new MintEvent(event.transaction.hash.toHexString().concat('-').concat(BigInt.fromI32(mints.length).toString()));
    ...
    if (event.params.to.toHexString() == pair.id) {
        ...
        let burn = new BurnEvent(event.transaction.hash.toHexString().concat('-').concat(BigInt.fromI32(burns.length).toString()));
    ...
    if (event.params.to.toHexString() == ADDRESS_ZERO && event.params.from.toHexString() == pair.id) {
        ...
        if (burns.length > 0) {
            ...
            if (currentBurn.needsComplete) {
                ...
                burn = new BurnEvent(event.transaction.hash.toHexString().concat('-').concat(BigInt.fromI32(burns.length).toString()));
                burn.transaction = transaction.id;
...
export function handleMint(event) {
...
export function handleBurn(event) {
    let transaction = Transaction.load(event.transaction.hash.toHexString());
    // safety check
    if (transaction === null) {
        return;
...
export function handleSwap(event) {
    let pair = Pair.load(event.address.toHexString());
    let token0 = Token.load(pair.token0);
    let token1 = Token.load(pair.token1);
    if (token0 === null || token1 === null) {
    ...
    let swap = new SwapEvent(event.transaction.hash.toHexString().concat('-').concat(BigInt.fromI32(swaps.length).toString()));
    // update swap event
    ...
    token0DayData.dailyVolumeUSD = token0DayData.dailyVolumeUSD.plus(amount0Total.times(token0.derivedETH).times(bundle.ethPrice));
    token0DayData.save();
    // swap specific updating
    token1DayData.dailyVolumeToken = token1DayData.dailyVolumeToken.plus(amount1Total);
    token1DayData.dailyVolumeETH = token1DayData.dailyVolumeETH.plus(amount1Total.times(token1.derivedETH));
    token1DayData.dailyVolumeUSD = token1DayData.dailyVolumeUSD.plus(amount1Total.times(token1.derivedETH).times(bundle.ethPrice));
    token1DayData.save();
}
