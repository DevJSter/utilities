...
const logComparison = (label: string, value1: string, value2: string, match: boolean) => {
  console.log(`🔍 ${label}:`);
 ...
 */
const deriveECDHSharedKey = (privateKeyBytes: Uint8Array, publicKeyBytes: Uint8Array): string => {
 ...
 */
export const signMessage = async (message: string, privateKeyBytes: Uint8Array): Promise<string> => {
...
export const sendSecureMessage = async (
  ...
  const signedPackage = JSON.stringify({
    ...
    from: Array.from(senderPublicKey).map(b => b.toString(16).padStart(2, '0')).join('')
  });
  ...
  const encryptedPackage = CryptoJS.AES.encrypt(signedPackage, sharedSecret).toString();
...
export const receiveSecureMessage = async (
  ...
  try {
    ...
    const sharedSecret = deriveECDHSharedKey(receiverPrivateKey, senderPublicKey);
    ...
    if (!signedPackage) {
      throw new Error('Decryption failed - invalid shared secret or corrupted data');
    }
    ...
    return {
      ...
      timestamp,
      from,
      error: isValid ? undefined : 'Signature verification failed - message may be tampered or fake'
    };
  } catch (error) {
    return {
      message: null,
      ...
      timestamp: '',
      from: '',
      error: `Decryption/Verification failed: ${error instanceof Error ? error.message : 'Unknown error'}`
    };
  }
};
...
const compareDigitalSignatures = async (
  ...
  console.log(`     Both Valid: ${sig1Valid && sig2Valid ? '✅ YES' : '❌ NO'}\n`);
  ...
  console.log(`     Format Valid: ${signature1.length === 128 && signature2.length === 128 ? '✅ YES' : '❌ NO'}\n`);
...
const createEthereumWallet = () => {
  ...
  logItem('Private Key', wallet.privateKey.substring(0, 16) + '...[REDACTED FOR SECURITY]', '🔐');
...
(async () => {
  try {
    ...
    const alicePubKeyHex = Array.from(aliceKeys.publicKeyBytes).map(b => b.toString(16).padStart(2, '0')).join('');
    const bobPubKeyHex = Array.from(bobKeys.publicKeyBytes).map(b => b.toString(16).padStart(2, '0')).join('');
    ...
    // Messages to send
    const secretMessage = "Meet me at the secret location at midnight. The code is: ALPHA-7749";
    ...
    await compareDigitalSignatures(testMessage, sig1, sig2, aliceKeys.publicKeyBytes);

    // Create Ethereum wallet
    const ethWallet = createEthereumWallet();

    // Final verification
    const allVerified = receivedMessage1.verified && receivedMessage2.verified && !tamperedResult.verified;
    ...
    if (allVerified) {
      ...
      console.log('   • Confidentiality: Only Bob can decrypt Alice\'s messages');
      console.log('   • Authenticity: Bob knows messages really came from Alice');
      console.log('   • Integrity: Any tampering is detected immediately');
      console.log('   • Non-repudiation: Alice cannot deny sending the messages');
    } else {
